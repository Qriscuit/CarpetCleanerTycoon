extends Node
## One GPU batch, no rigid bodies. Only airborne/sliding clumps are simulated.
...
func reset() -> void:
	...
	vacuum_elapsed = 0.0
	vacuum_origins.clear()
	for material in surface_materials:
		material.set_shader_parameter("dust_strength", rug_definition.dust_strength)
	...
	for i in initial.size():
		positions.append(initial[i].origin)
		velocities.append(Vector3.ZERO)
		cleared.append(false)
		cooldown.append(0.0)
		moving.append(false)
		growth.append(initial_growth[i])
		awakened.append(false)
		credited.append(false)
...
func refresh_visible_clumps() -> void:
	...
	for i in positions.size():
		...
		var radius := maxf(radii[i] * growth[i] * 2.0, 0.12)
		var visible := true
		for plane in planes:
		...
		if not visible:
			continue
		var pose := initial[i]
		pose.basis = pose.basis.scaled(Vector3.ONE * growth[i])
...
func animate_vacuum(delta: float) -> void:
	...
	for material in surface_materials:
		material.set_shader_parameter("dust_strength", rug_definition.dust_strength * (1.0 - smoothstep(0.15, 1.2, vacuum_elapsed)))
	for i in positions.size():
		...
		growth[i] = 1.0 - smoothstep(0.72, 1.0, t)
	request_render()
